...
#   NO_LINEAGE         if set to 1, omit the lineage-telemetry plugin from the
#                      generated ConfigMap (parsers stay — legal alone; the
#                      plugin declares RequiresAny{parsers}, not the reverse).
#                      The sidecar still proxies traffic, emits zero spans.
#                      NO_OTEL=1 NO_LINEAGE=1 together = lineage fully off for
#                      this app (see lineage-switch.sh).
set -euo pipefail
...
# Empty under NO_LINEAGE=1: the parsers remain (content-gated, harmless alone)
# and the sidecar becomes a pure proxy that emits nothing.
lineage_plugin=""
if [ "$NO_LINEAGE" != "1" ]; then
  lineage_plugin='
...
# app=${NAME} self_id=${SELF_ID} image=${IMAGE} otel=$([ "$NO_OTEL" = 1 ] && echo off || echo on) lineage=$([ "$NO_LINEAGE" = 1 ] && echo off || echo on)
---
apiVersion: v1
kind: ConfigMap
metadata:
