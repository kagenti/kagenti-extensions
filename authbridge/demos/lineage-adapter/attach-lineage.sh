...
# app=${NAME} self_id=${SELF_ID} image=${IMAGE} otel=$([ "$NO_OTEL" = 1 ] && echo off || echo on) lineage=$([ "$NO_LINEAGE" = 1 ] && echo off || echo on)${pvc_manifest}
---
apiVersion: v1
kind: ConfigMap
metadata:
      ...
      volumes:
        ...
        - name: authbridge-runtime
          configMap:
            name: authbridge-lineage-config-${NAME}
            items:
              - { key: config.yaml, path: config.yaml }${pvc_volume_yaml}
EOF
