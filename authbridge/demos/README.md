...
| **[Lineage Adapter](lineage-adapter/README.md)** | Reference | Data-governance lineage: wrap any Kagenti app image with a propagate-only OTel shim, attach the envoy-sidecar with the `lineage-telemetry` plugin, and get a correct per-request interaction forest in the DG service (sibling repo `lab-data-governance`). Fleet catalog + attach/verify scripts + per-app runbook | kubectl + scripts |
