...
# ---- optional command override (see APP_COMMAND in the header) ----
# Emitted verbatim as command: — replaces the image ENTRYPOINT+CMD with the
...
for kv in $ENV_VARS; do
  ...
  k="${kv%%=*}"; v="${kv#*=}"
  yaml_safe "ENV_VARS value for $k" "$v"
  env_block="${env_block}
            - { name: ${k}, value: \"${v}\" }"
done
