...
does so by forwarding one header, `traceparent`. So the app image gets one
...
> `lineage-telemetry` and logs `unknown plugin`. [RECIPE.md](RECIPE.md) step 1
> builds and loads `authbridge-envoy` + `proxy-init` from this repo.

**Start here:** [RECIPE.md](RECIPE.md) — five commands, expected output, back
...
is visibly its own trace rather than silently misattributed. The spans say
...
collision, `APP_CONTAINER` names a real container), applies the ConfigMap,
...
out, `kubectl rollout undo deploy/<name>` (the patch is one revision), then
delete `authbridge-lineage-config-<name>`.
...
| attach | `DEPLOY=<name> APP_CONTAINER=<c> APP_IMAGE=…-otel:latest ./sidecar-patch.sh` | Deployment | the ConfigMap, the patch, four preconditions, the rollout wait |
...
`APP_CONTAINER`, `APP_IMAGE`, `OUTBOUND_PORTS_EXCLUDE`, `SIDECAR_IMAGE`,
...
BAKE — once per app image                 ATTACH — once per Deployment
  build-otel-shim.sh <app>:latest           sidecar-patch.sh DEPLOY=<name> [APP_CONTAINER= APP_IMAGE=]
    ├─ container-runtime.sh   podman/docker, kind load     ├─ checks    Deployment · envoy-config · ports · container
...
| `sidecar-patch.sh` refuses: "already declares containerPort …" | Enrolled workload, or the app itself listens on 9090/15123/15124. Namespace route for the former; the latter cannot be adopted. |
