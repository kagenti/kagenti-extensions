...
present the hop is parented on whatever `traceparent` the wire carried — which
may be nothing — and records `lineage.parent.source=wire`. The forwarded
`traceparent` itself is never modified, and nothing guesses: a hop without
context is *visibly* unparented rather than attached to a plausible caller.

So there is exactly one `wire` per well-propagated trace, at the entry, where
the caller supplied the first `traceparent`. Every other `wire` marks a pod
that did not propagate. The subtree beneath it fragments out of its caller's
...
logs; propagation is then off and the trace shows it (`wire`), which is the
...
recipe lacked the `urllib3` instrumentor. One `wire` in the wrong place is the
whole signal.
...
- **Non-HTTP protocols cannot pass through the outbound listener at all.**
  ...
  lineage either way. S3 is HTTP and stays in.
