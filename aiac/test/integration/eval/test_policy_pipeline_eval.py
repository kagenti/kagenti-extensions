...
pytestmark = pytest.mark.integration_extended

HERE = Path(__file__).resolve().parent  # test/integration/eval/
...
def pipeline() -> dict[str, dict]:
    ...
    for name, scenario in SCENARIOS.items():
        ...
        expected = [
            rego_dir / f"{slug}.{direction}.rego"
            for slug in (a.replace("-", "_") for a in scenario.AGENTS)
            for direction in ("inbound", "outbound")
            if slug not in allow_missing
        ]
...
def test_inbound(
    ...
    allowed = opa_eval([rego], f"data.authz.{slug}.inbound.allow", {"subject": subject})
...
def test_outbound(
    ...
    role = scenario.USERS[subject]
    reasoning_by_scope = pipeline[scenario_name]["reasoning_by_scope"]
    reasoning_by_agent_role = pipeline[scenario_name]["reasoning_by_agent_role"]
    ...
    slug = agent_id.replace("-", "_")
    rego = pipeline[scenario_name]["rego_dir"] / f"{slug}.outbound.rego"
    if not rego.is_file():
        record_property("output", None)
    ...
    allowed = opa_eval(
        ...
        {"agent_id": slug, "subject": subject, "target": target, "function_name": fn},
    )
    record_property("output", allowed)
...
def test_outbound_unknown_target_denied(pipeline: dict[str, dict], scenario_name: str) -> None:
    ...
    allowed = opa_eval(
        [rego, HERE / "probe_eval.rego"],
        "data.probe.outbound_eval.allow",
        {"agent_id": slug, "subject": subject, "target": "unknown-target", "function_name": "Some.Op"},
    )
...
def test_outbound_soft_match_not_overbroad(pipeline: dict[str, dict], scenario_name: str) -> None:
    ...
    allowed = opa_eval(
        [rego, HERE / "probe_eval.rego"],
        "data.probe.outbound_eval.allow",
        {"agent_id": slug, "subject": subject, "target": target, "function_name": "delete_everything"},
    )
    assert allowed is False
...
def test_grant_set_matches_truth_table(pipeline: dict[str, dict], scenario_name: str, gate: str) -> None:
    ...
    cannot see."""
    scenario = SCENARIOS[scenario_name]
    got = grant_sets(scenario, pipeline[scenario_name]["rules"])[gate]
