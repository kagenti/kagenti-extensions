...
| `policy-eval-scenarios` — `test_policy_pipeline_eval.py` + guardrail tests | Generalized evaluation suite extending `policy-pipeline`'s single-agent/single-tool proof to five scenarios: baseline-scale (many entities, names decoupled from roles, one agent→agent delegation grant), missing-details (emergent unreachability/zero-access under deny-by-default, one genuinely multi-interpretable clause, wildcard-grant expansion), adversarial-authoring (misleading names/descriptions, an identity/boundary-confusion probe), and ambiguous-and-contradictory / adversarial-injection-and-edge-cases (whole-document `xfail` checks against the PRB directly, no Keycloak or `opa`). The three heavy scenarios (`@pytest.mark.integration_extended`) assert full per-cell `opa eval` truth tables; the two light scenarios (`@pytest.mark.integration`) assert PRB-level rejection. | [integration-test/policy-eval-scenarios.md](integration-test/policy-eval-scenarios.md) |
