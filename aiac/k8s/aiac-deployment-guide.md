...
> is the Dockerfile's own directory. The `opa`, `policy-store`, and `agent` images bundle the
...
# Policy Store — context: aiac/src/ (bundles the aiac package)
...
# 3. Policy Store — needs the aiac-system namespace
kubectl apply -f aiac/k8s/policy-store-statefulset.yaml

# 4. Agent — aiac-init waits for NATS + Interface Pod + Policy Store to be healthy
